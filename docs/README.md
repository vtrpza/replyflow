# Documentation

## Root-level docs

| File | Description |
|------|-------------|
| [README.md](../README.md) | Project overview, quick start, and product direction |
| [AGENTS.md](../AGENTS.md) | Agent guidelines, code style, and architecture patterns |
| [CONTRIBUTING.md](../CONTRIBUTING.md) | Branch strategy, commit convention, and PR checklist |
| [CHANGELOG.md](../CHANGELOG.md) | Version history following Keep a Changelog |

## docs/ folder

| File | Description |
|------|-------------|
| [DEPLOYMENT.md](./DEPLOYMENT.md) | Fly.io deployment runbook and SQLite persistence |
| [SENTRY.md](./SENTRY.md) | Sentry error monitoring setup and configuration |
| [VERSIONING.md](./VERSIONING.md) | Semantic versioning policy and release cadence |
| [IMPLEMENTATION_RUNBOOK.md](./IMPLEMENTATION_RUNBOOK.md) | Detailed implementation notes for sources, connectors, and sync |
| [CONNECTOR_EXPANSION_PLAN.md](./CONNECTOR_EXPANSION_PLAN.md) | Connector expansion strategy |
